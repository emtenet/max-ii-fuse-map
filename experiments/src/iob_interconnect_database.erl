-module(iob_interconnect_database).
...
build_keys(Density, [Fuse | Fuses], Muxes0, Keys) ->
    case fuse_map:to_name(Fuse, Density) of
        {ok, {Block = {iob, _, _}, {interconnect, Index}, direct_link}} ->
            Key = {Block, Index, direct_link},
            build_keys(Density, Fuses, Muxes0, [Key | Keys]);

        {ok, {Block = {iob, _, _}, {interconnect, Index}, from3, Mux3}} ->
            Mux = {Block, Index},
        ...
        {ok, {Block = {iob, _, _}, {interconnect, Index}, from4, Mux4}} ->
            Mux = {Block, Index},
            case maps:take(Mux, Muxes0) of
                {{from3, Mux3}, Muxes} ->
                    Key = {Block, Index, {Mux4, Mux3}},
...
build_maybe(Density, Block = {iob, X, Y}, Index, From, Fuses) ->
...
build_location(Density, X, Y, Fuse) ->
    case fuse_map:to_location(Fuse, Density) of
        {XX, YY, N, I, side, S} when XX =:= X orelse YY =:= Y ->
            {true, {S, N, I}};
...
open([{NextBlock = {iob, _, _}, NextIndex} | Lines],
     Blocks, Block, Index, Mux) ->
    case Blocks of
        #{Block := Indexes} ->
            open(
