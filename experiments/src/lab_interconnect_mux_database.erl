...
-type entry() :: {lab(), interconnect(), select(), from()}.
...
run(I, Mux4, Mux3, Es) ->
    ...
    case detect(Fs) of
        ...
        {Axis, false, false, false} ->
            throw({I, Mux4, Mux3, variable_x_y_n, Axis});
        ...
        {Axis, false, false, N} ->
            throw({I, Mux4, Mux3, variable_x_y, Axis, N});
...
detect([]) ->
    false;
detect([{{lab, X, Y}, _, _, {Axis, XX, YY, 0, N}} | Es]) ->
...
detect([], Axis, X, Y, N) ->
    {Axis, X, Y, N};
detect([{{lab, X, Y}, _, _, {Axis, XX, YY, 0, N}} | Es], Axis0, X0, Y0, N0) ->
    detect(
        Es,
...
table_max([], MaxX, MaxY) ->
    {MaxX, MaxY};
table_max([{{lab, X, Y}, _, _, _} | Es], MaxX, MaxY) ->
...
table_cell(_, _, [], With) ->
    With(undefined);
table_cell(X, Y, [{{lab, X, Y}, _, _, From} | _], With) ->
    With(From);
...
update_check([], Add) ->
    {true, Add};
update_check([{LAB, IC, Select, Exist} | _], {LAB, IC, Select, Exist}) ->
    false;
update_check([{LAB, IC, Select, Exist} | _], {LAB, IC, Select, Add}) ->
    throw({update, LAB, IC, Select, exist, Exist, add, Add});
...
merge([M | Ls], [M | Rs], Ms) ->
    merge(Ls, Rs, [M | Ms]);
merge([{LAB, IC, Select, L} | _], [{LAB, IC, Select, R} | _], _) ->
    throw({merge, LAB, IC, Select, left, L, right, R}).
